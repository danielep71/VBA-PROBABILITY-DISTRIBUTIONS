...
| Measured head-to-head comparison against Excel | — | — | ✅ |
| Evidence cryptographically bound to source | — | — | ✅ |
| Thresholds validated on a held-out sample | — | — | ✅ |
...
|---|---|---:|---:|---|
| Standard normal, `z = 8` | `1-NORM.S.DIST(8,TRUE)` | **0** | full | `6.22e-16` |
| Standard normal, `z = 15` | `1-NORM.S.DIST(15,TRUE)` | **0** | full | `3.67e-51` |
| Exponential, `x = 50` | `1-EXPON.DIST(50,1,TRUE)` | **0** | full | `1.93e-22` |
| Weibull, `x = 10` | `1-WEIBULL.DIST(10,2,1,TRUE)` | **0** | 13 | `3.72e-44` |
| Gamma, `x = 60` | `1-GAMMA.DIST(60,2,1,TRUE)` | **0** | 14 | `5.34e-25` |
| Poisson, `k = 60` | `1-POISSON.DIST(60,2,TRUE)` | **0** | 14 | `6.35e-67` |
| Binomial, `k = 90` | `1-BINOM.DIST(90,100,0.5,TRUE)` | **0** | 13 | `1.66e-18` |
| Lognormal, deep tail | `1-LOGNORM.DIST(2981,0,1,TRUE)` | **0** | 15 | `6.22e-16` |

**Zero correct digits** is not a rounding difference. At `z = 15` the expression
returns exactly `0` against a true value of `3.67e-51`. The library returns the
right answer:

```excel
=K_STATS_NormalStandard_Survival(15)     ' 3.67096619931E-51
```

A further four common needs have **no Excel function at all**: inverse survival,
uniform and discrete-uniform distribution functions, and log-mass for any
discrete family.
...
The main accuracy grid currently includes external contracts for all six discrete
families:
...
Every one of these contracts is `validated and frozen`: its threshold was measured
on the main grid and then confirmed on an independent holdout of fresh parameter
sets that were not used to set any threshold. Discrete Uniform was the last to
join — grid evidence in `f8ea15e`, contracts frozen in `72aaa92` (2026-07-23).
...
Each contract in [`benchmark/accuracy_summary.md`](benchmark/accuracy_summary.md)
carries one of five verdicts:
...
| 🧪 **CHARACTERIZATION ONLY** | measured behavior, not used as a pass/fail claim |
| ⏳ **PENDING** | an active contract that is not fully evaluated |

`PENDING` blocks the gate exactly as `FAIL` does. An active contract that cannot
be evaluated — because an observation is missing or a reference helper is
unavailable — is never allowed to pass quietly.
...
│  └─ social3.jpg
├─ benchmark/
│  ├─ beta_f_unbalanced/        # study: unbalanced Beta/F shapes
│  ├─ cdf_large_shape/          # study: incomplete gamma/beta at large shape
│  ├─ delta_seam_study/         # study: Stirling-error seam
│  ├─ density_large_shape/      # study: the CR-P1-01 density defect
│  ├─ envelope_probe/           # study: measured df envelopes
│  ├─ excel_comparison/         # measured head-to-head against Excel
│  ├─ f_envelope/               # study: superseded, retained as record
│  ├─ holdout/                  # independent validation sample
│  ├─ inverse_probe/            # study: inverse convergence domains
│  ├─ logbeta_study/            # study: LogBeta dispatch
│  ├─ survival_boundary/        # study: survival tail boundaries
│  ├─ t_density_large_df/       # study: Student t density at large df
│  ├─ tchi_large_df/            # study: t and chi-square at large df
│  ├─ density_helpers/          # study: shared log-density kernels
│  ├─ holdout_older/            # earlier holdout sample, retained
...
- ✅ regime-aware validation across the complete discrete surface;
- ✅ exported observations bound to the source revision and module hashes
  (`benchmark/observation_manifest.json`, SHA-256 per `.bas`);
- ✅ direct comparison artifacts against 50-digit references, and a measured
  head-to-head comparison against Excel's native functions
  (`benchmark/excel_comparison/`);
- ✅ machine-readable accuracy and regression artifacts, regenerated and
  diff-checked in CI.

**Planned:**

- publish repeatable performance benchmarks with recorded environments;
- add API/documentation consistency checks beyond the current source-claim
  checker;
- strengthen automated exported-module static checks (module-name uniqueness,
  frozen public API inventory, test-section registration);
- bring the independent holdout grid under the same source-provenance binding
  as the main grid.
