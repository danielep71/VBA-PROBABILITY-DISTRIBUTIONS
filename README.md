...
| **External accuracy pipeline** | Regime-aware contracts cover all six discrete families — Binomial, Poisson, Geometric, Negative Binomial, Hypergeometric, and Discrete Uniform — each validated on an independent holdout and frozen |
...
> [!CAUTION]
> The external benchmark grid and the VBA regression harness are complementary but not identical: the harness asserts behavior and error contracts inside Excel, while the grid measures observed values against 50-digit references. All six discrete families are present in both. Discrete Uniform entered the external grid in `f8ea15e` and its contracts were frozen in `72aaa92` (2026-07-23).
>
