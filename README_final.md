...
[![Excel CI](https://img.shields.io/badge/Excel_Regression-835_Assertions-d97706?style=for-the-badge&logo=githubactions&logoColor=white)](docs/EXCEL_VBA_CI.md)
[![Accuracy Contracts](https://img.shields.io/badge/Accuracy_Contracts-161_Measured-0f766e?style=for-the-badge)](benchmark/accuracy_summary.md)
...
| **835** | deterministic VBA assertions, run in Excel on every push |
| **161** | active accuracy contracts measured against 50-digit references |
| **80** | of those additionally validated on an **independent holdout** |
| **1 905** | observation rows bound to source by SHA-256 |
| **0** | failing contracts, pending contracts, or known silent-wrong paths |
...
The library has been through several independent code reviews. Two of them found
**P1 numerical defects in production code**; both are closed, measured, and now
protected by frozen accuracy contracts.

| Finding | What was wrong | Resolution |
|---|---|---|
| **CR-P1-01** — large-shape density cancellation | Gamma, Chi-square, Beta and F densities subtracted two quantities of size `shape·log(shape)`. At shape `1E16` the log-density was wrong by up to **203**, i.e. the density itself off by a factor of `1E183` — silently, inside the accepted domain | Rerouted through shared Loader deviance and Stirling-error kernels. Worst absolute log-density error fell from **83 to 1.6E-5** across all four families |
| **CR-P1-02** — incomplete-function prefactor cancellation | The same defect class in the incomplete gamma and beta prefactors, affecting every CDF, survival and inverse outside the Normal family. Wrong by `e^46` at shape `1E16` | Prefactors derived from the stable log-density kernels, with a measured regime dispatch. Cumulative accuracy improved by **145×** after a matching stopping-criterion change |
| **CR-P1-03** — stale independent evidence | Contract thresholds were tightened from main-grid measurements without re-exporting the holdout, so the committed summary reported PASS against superseded thresholds | Holdout re-exported; thresholds stood unchanged. CI now runs the holdout analyzer on every push |

Alongside those:

| Development | Current state |
|---|---|
| **Measured accuracy envelopes** | Every enveloped surface rejects beyond its **measured** boundary with `#NUM!` rather than returning a silently inaccurate value. The F envelope moved from `1E5` to `1E10` once CR-P1-02 removed the cancellation the original bound had actually been measuring |
| **Complete discrete probability layer** | Binomial, Poisson, Geometric, Negative Binomial, Hypergeometric and Discrete Uniform — PMF/LogPMF/CDF/SF/inverse/moments, with Loader-style stable mass calculations and direct right tails |
| **Evidence bound to source** | `observation_manifest.json` binds a SHA-256 of every `.bas` module, the observation grid and the contract registry. The gate hard-fails if source drifts from the evidence |
| **Independent holdout** | 80 contracts validated against points held back from threshold derivation — which has been the binding constraint on three separate occasions |
| **Self-testing CI** | The hosted gate runs the evaluator unit tests, the strict accuracy gate, the degradation test, generated-artifact freshness diffs, the source-claim checker, and the independent holdout |

The latest generated benchmark summary is in [`benchmark/accuracy_summary.md`](benchmark/accuracy_summary.md), which distinguishes:
...
> [!CAUTION]
> The external benchmark grid and the VBA regression harness are complementary but not identical: the harness asserts behavior and error contracts inside Excel, while the grid measures observed values against 50-digit references.
>
> The hosted accuracy workflow evaluates the **committed observation grid**. It does not execute desktop Excel or regenerate observations from the current VBA source, so numerical source changes require a fresh Excel export before the benchmark evidence is treated as current. The manifest exists to make a stale export impossible to commit unnoticed.
