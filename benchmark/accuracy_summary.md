...
> **Verdict tally** — FAIL: 0, KNOWN LIMITATION: 1, PENDING: 0.

> States: **PASS** meets the contract; **FAIL** exceeds it and must be investigated; **KNOWN LIMITATION** is a documented defect tracked in `accuracy_contracts.csv` (does not read as green); **PENDING** is not yet measured. Rows marked *below harness precision* have relative claims tighter than a 15-16 digit CSV round-trip can verify and are not failures.
