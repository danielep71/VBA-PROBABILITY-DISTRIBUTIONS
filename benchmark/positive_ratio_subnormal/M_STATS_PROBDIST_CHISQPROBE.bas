Attribute VB_Name = "M_STATS_PROBDIST_CHISQPROBE"
Option Explicit
...
'------------------------------------------------------------------------------
' PURPOSE
'   Phase A2 exporter for ICR-P1-01A (#13), Chi-square arm.
'
...
'
' USAGE
'   Run Probe_ChiSquarePositiveRatio, then validate with
...
Public Sub Probe_ChiSquarePositiveRatio()
'
'==============================================================================
' Probe_ChiSquarePositiveRatio
'------------------------------------------------------------------------------
' PURPOSE
...
'------------------------------------------------------------------------------
    ...
    MsgBox "Chi-square positive-ratio probe complete: " & Count & _
           " construction(s), " & Rows & " rows written to" & _
           vbCrLf & Path, vbInformation, "Chi-square probe"
    Exit Sub
...
Err_Handler:
    ...
    MsgBox "Chi-square positive-ratio probe failed: " & Err.Description, _
           vbExclamation, "Chi-square probe"
End Sub
...
'------------------------------------------------------------------------------
    Select Case Surface
        Case "survival"
            ShapeSlicesFor = Array(0.0002, 0.001, 0.002, 0.004, 1#)
        Case Else
            ShapeSlicesFor = Array(0.002, 0.02, 0.2, 0.5, 1#, 1.5, 2#)
    End Select
End Function
...
'------------------------------------------------------------------------------
    ...
    'error is introduced. Isolates downstream behaviour.
        AddPoint Ids, Cons, Bits, Xs, Scales, Count, _
                 "c_landmark_k" & K, "landmark", K, 2# * N * MinSub, 2#
    ...
    'midpoint.
        AddPoint Ids, Cons, Bits, Xs, Scales, Count, _
                 "c_tie_k" & K, "transform_stress", K, _
                 (2# * N + 1#) * MinSub, 2#
    Next B
'------------------------------------------------------------------------------
' HARD UNDERFLOW
'------------------------------------------------------------------------------
    ...
    'ordinary as 1E-300 and 1E+300.
        AddPoint Ids, Cons, Bits, Xs, Scales, Count, _
                 "c_underflow_minsub", "underflow", 0, MinSub, 2#
'------------------------------------------------------------------------------
' DECIMAL TWINS
'------------------------------------------------------------------------------
    ...
    'is suspect. The analyzer pairs them by stripping the twin_ prefix
        AddPoint Ids, Cons, Bits, Xs, Scales, Count, _
                 "twin_c_landmark_k8", "decimal_twin", 8, _
                 1.26480805335359E-321, 2#
        AddPoint Ids, Cons, Bits, Xs, Scales, Count, _
                 "twin_c_landmark_k32", "decimal_twin", 32, _
                 2.12199579096527E-314, 2#
End Sub
...
'------------------------------------------------------------------------------
    Picked = Application.GetSaveAsFilename( _
        InitialFileName:="chisquare_probe.csv", _
        FileFilter:="Probe output (*.csv),*.csv", _
        Title:="Save the Chi-square positive-ratio probe")
