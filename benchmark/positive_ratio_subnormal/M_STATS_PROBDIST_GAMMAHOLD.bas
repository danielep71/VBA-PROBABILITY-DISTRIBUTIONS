Attribute VB_Name = "M_STATS_PROBDIST_GAMMAHOLD"
Option Explicit
...
'------------------------------------------------------------------------------
' PURPOSE
'   Independent HOLDOUT exporter for ICR-P1-01A (#13), Gamma arm.
'
'   Same constructions and same schema as M_STATS_PROBDIST_GAMMAPROBE, but a
'   structurally disjoint point set. Everything measured in A1 and A2 is
'   fitting evidence; this is what tests whether the 48-bit density and
'   40-bit cumulative crossovers survive contact with points that had no
'   hand in choosing them.
'
' WHY DISJOINTNESS IS STRUCTURAL, NOT JUST DIFFERENT
'   Every strict threshold in this repository has been rejected on holdout
'   evidence, and an independent holdout has found a worse point than its
'   fitting grid every time it has been run. A holdout that merely picks
'   other round numbers on the same ladder does not test that.
'
'   Buckets: the fitting arm used an even ladder, 52 down to 1 in steps of
'   four. This uses 50, 46, 41, 37, 33, 29, 25, 21, 17, 13, 9, 5, 3 - none
'   shared, and eleven of thirteen odd, so they sit at binary exponents the
'   fitting set never touched.
'
'   Shapes: the fitting arm used round decimals. These are derived from
'   irrational constants, so their mantissas carry no structure the fitting
'   set shares; six of eight have the low mantissa bit set.
'
'   Constructions were re-verified at the odd buckets before this module was
'   written: every landmark and stress is exactly representable, lands on its
'   declared offset, and carries exactly k significand bits. The recipe had
'   only ever been exercised on the even ladder.
'
...
'
' USAGE
'   Run Probe_GammaHoldout and choose an output path. Validate with
...
'------------------------------------------------------------------------------
    ...
    MsgBox "Gamma positive-ratio HOLDOUT complete: " & Count & _
           " construction(s), " & Rows & " rows written to" & _
           vbCrLf & Path, vbInformation, "Gamma holdout"
    Exit Sub
...
'------------------------------------------------------------------------------
    Select Case Surface
        Case "survival"
    'Survival needs its own set: five of the eight below are useless for it,
    'because the lower tail falls under binary64 resolution relative to 1 and
    'Q rounds to exactly 1 across the whole ladder. Eligibility is fixed in
    'HOLDOUT_DESIGN.md as min(P, Q) > 2 ^ -20 on the exact reference at every
    'holdout bucket - a statement about conditioning alone, evaluated before
    'any implementation runs, so it cannot encode which branch wins.
            ShapeSlicesFor = Array(0.00036787944117144, _
                                   0.00070710678118655, _
                                   0.00157079632679490, _
                                   0.0072973525693, _
                                   0.0141421356237310)
        Case Else
    'Derived from irrational constants: alpha, 1/1000e, sqrt2/100, ln2/10,
    'pi/10, phi - 1, sqrt2/2, e/3. None shares a value with the fitting set
            ShapeSlicesFor = Array(0.0072973525693, _
                                   0.00036787944117144, _
                                   0.014142135623731, _
                                   0.069314718055995, _
                                   0.31415926535898, _
                                   0.61803398874989, _
...
'------------------------------------------------------------------------------
    ...
    'N = 2 ^ (K - 1) by doubling; exact, and never large enough to lose bits
        ...
        AddPoint Ids, Cons, Bits, Xs, Scales, Count, _
                 "gh_landmark_k" & K, "landmark", K, N * MinSub, 1#
        AddPoint Ids, Cons, Bits, Xs, Scales, Count, _
                 "gh_s13_k" & K, "transform_stress", K, _
                 (3# * N + 1#) * MinSub, 3#
        AddPoint Ids, Cons, Bits, Xs, Scales, Count, _
                 "gh_s12_k" & K, "transform_stress", K, _
                 (2# * N + 1#) * MinSub, 2#
        AddPoint Ids, Cons, Bits, Xs, Scales, Count, _
                 "gh_s23_k" & K, "transform_stress", K, _
                 (3# * N + 2#) * MinSub, 3#
    Next B
'------------------------------------------------------------------------------
' HARD UNDERFLOW
'------------------------------------------------------------------------------
    ...
    'become indistinguishable from the support boundary
        AddPoint Ids, Cons, Bits, Xs, Scales, Count, _
                 "gh_underflow_minsub", "underflow", 0, MinSub, 4#
        AddPoint Ids, Cons, Bits, Xs, Scales, Count, _
                 "gh_underflow_wide", "underflow", 0, 1E-300, 1E+300
        AddPoint Ids, Cons, Bits, Xs, Scales, Count, _
                 "gh_underflow_deep", "underflow", 0, 1E-200, 1E+200
'------------------------------------------------------------------------------
' DECIMAL TWINS
'------------------------------------------------------------------------------
    ...
    'is suspect. The analyzer pairs them by stripping the twin_ prefix
        AddPoint Ids, Cons, Bits, Xs, Scales, Count, _
                 "twin_gh_landmark_k9", "decimal_twin", 9, _
                 1.26480805335359E-321, 1#
        AddPoint Ids, Cons, Bits, Xs, Scales, Count, _
                 "twin_gh_landmark_k33", "decimal_twin", 33, _
                 2.12199579096527E-314, 1#
End Sub
...
'------------------------------------------------------------------------------
    Picked = Application.GetSaveAsFilename( _
        InitialFileName:="gamma_holdout.csv", _
        FileFilter:="Probe output (*.csv),*.csv", _
        Title:="Save the Gamma positive-ratio HOLDOUT")
