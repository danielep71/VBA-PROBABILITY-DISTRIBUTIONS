...
For the shape regime confirmed by real Excel, use the normalized series:
...
This avoids both `1 / A` and the small-shape Loader/Stirling prefactor.
`LogGamma1p(A)` is the existing project kernel; no `1 + A` increment may be
formed in the small-shape series regime. The Loader prefactor remains the
preferred production path near the distribution body at large shape.

The binary64 candidate mirror predicts relative error below `5E-16` at every
existing `X = 0.5`, `Shape <= 0.1` fitting point. That prediction is not
closure evidence; the post-change values must be exported from real VBA.
