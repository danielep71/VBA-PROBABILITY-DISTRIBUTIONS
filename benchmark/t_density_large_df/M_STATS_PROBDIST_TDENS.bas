Attribute VB_Name = "M_STATS_PROBDIST_TDENS"
Option Explicit

'==============================================================================
' M_STATS_PROBDIST_TDENS
'------------------------------------------------------------------------------
' PURPOSE
'   Self-contained export macro for the large-shape density study (CR-P1-01).
'   Fills observed_vba in t_density_large_df_grid.csv for the Gamma, Chi-square,
'   Student t density across df from 1E2 to 1E20, so the accuracy of the
'   LogGammaHalfDiff asymptotic path at large df can be
'   measured from real VBA and set beside the stable Loader target.
'
' USAGE
'   Run Export_TDensityLargeDf and pick t_density_large_df_grid.csv.
'
...
'   two-parameter Student t density). This macro writes observed_vba (column
...
Public Sub Export_TDensityLargeDf()
'
'==============================================================================
' Export_TDensityLargeDf
'------------------------------------------------------------------------------
' PURPOSE
...
'------------------------------------------------------------------------------
    'Row 0 is the header; data starts at row 1
        For I = 1 To UBound(Lines)
            ...
            Cols(8) = EvaluateTDensity(Trim$(Cols(0)), A1, A2, A3)
...
'------------------------------------------------------------------------------
    ...
    MsgBox "Large-shape density study complete: " & Filled & _
           ...
           vbInformation, "t density large-df"
    Exit Sub
...
Err_Handler:
    ...
    MsgBox "Large-shape density study failed: " & Err.Description, _
           vbExclamation, "t density large-df"
End Sub


Private Function EvaluateTDensity( _
...
'------------------------------------------------------------------------------
' PURPOSE
'   Dispatches the public t density function and returns a string token: a
...
'------------------------------------------------------------------------------
    ...
    ElseIf Not IsNumeric(V) Then
        EvaluateTDensity = "ERROR"
    Else
        EvaluateTDensity = FormatFullPrecision(CDbl(V))
    End If
    Exit Function
...
'------------------------------------------------------------------------------
' PURPOSE
'   Returns a usable LOCAL path to t_density_large_df_grid.csv, or an empty
...
'------------------------------------------------------------------------------
    ...
    If Len(BookPath) > 0 And LCase$(Left$(BookPath, 4)) <> "http" Then
        Candidate = BookPath & Application.PathSeparator & "t_density_large_df_grid.csv"
    ...
    MsgBox "Could not locate t_density_large_df_grid.csv automatically " & _
           "(the workbook may be on OneDrive/SharePoint). Please select it.", _
           vbInformation, "Locate t density grid"
    Picked = Application.GetOpenFilename( _
        FileFilter:="t density grid (*.csv),*.csv", _
        Title:="Select t_density_large_df_grid.csv")
