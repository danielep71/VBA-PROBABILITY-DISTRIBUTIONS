Attribute VB_Name = "M_STATS_PROBDIST_FENVBL"
Option Explicit

'==============================================================================
' M_STATS_PROBDIST_FENVBL
'------------------------------------------------------------------------------
' PURPOSE
'   Self-contained export macro for the F accuracy-envelope study in its
'   BOTH-LARGE orientation. Fills observed_vba in f_envelope_bothlarge_grid.csv
'   for F_Cumulative and F_Survival.
'
' WHY THIS EXISTS
'   The one-large sweep (one df large, the other near 1) is not the worst case.
'   When BOTH incomplete-beta parameters are large the continued fraction
'   degrades earlier. This sweep covers d1 = d2 and d1 = df with d2 = 3 * df
'   across the df range, so the strict F envelope can be taken as the MINIMUM
'   boundary over all orientations rather than the one-large case alone.
'
' USAGE
'   Run Export_FEnvelopeBothLarge and pick f_envelope_bothlarge_grid.csv.
'
...
Public Sub Export_FEnvelopeBothLarge()
'
'==============================================================================
' Export_FEnvelopeBothLarge
'------------------------------------------------------------------------------
' PURPOSE
'   Fills the observed_vba column of the both-large F envelope grid in place.
'
' BEHAVIOR
...
'------------------------------------------------------------------------------
    ...
    MsgBox "F envelope BOTH-LARGE study complete: " & Filled & _
           " observation(s) written to" & vbCrLf & Path, _
           vbInformation, "F envelope both-large"
    Exit Sub
...
Err_Handler:
    ...
    Close #FileNum
    Close #OutNum
    MsgBox "F envelope both-large study failed: " & Err.Description, _
           vbExclamation, "F envelope both-large"
End Sub
...
'------------------------------------------------------------------------------
' PURPOSE
'   Returns a usable LOCAL path to f_envelope_bothlarge_grid.csv, or an empty
...
'------------------------------------------------------------------------------
    ...
    If Len(BookPath) > 0 And LCase$(Left$(BookPath, 4)) <> "http" Then
        Candidate = BookPath & Application.PathSeparator & "f_envelope_bothlarge_grid.csv"
    ...
    MsgBox "Could not locate f_envelope_bothlarge_grid.csv automatically " & _
           "(the workbook may be on OneDrive/SharePoint). Please select it.", _
           vbInformation, "Locate F envelope both-large grid"
    Picked = Application.GetOpenFilename( _
        FileFilter:="F envelope grid (*.csv),*.csv", _
        Title:="Select f_envelope_bothlarge_grid.csv")
