Attribute VB_Name = "M_STATS_PROBDIST_FENV"
Option Explicit

'==============================================================================
' M_STATS_PROBDIST_FENV
'------------------------------------------------------------------------------
' PURPOSE
'   Self-contained export macro for the F accuracy-envelope study. Fills
'   observed_vba in f_envelope_grid.csv for F_Cumulative and F_Survival across a
...
'
' USAGE
'   Run Export_FEnvelope and pick f_envelope_grid.csv in the dialog.
'
...
Public Sub Export_FEnvelope()
'
'==============================================================================
' Export_FEnvelope
'------------------------------------------------------------------------------
' PURPOSE
'   Fills the observed_vba column of the F envelope grid in place.
'
' BEHAVIOR
...
'------------------------------------------------------------------------------
    ...
    MsgBox "F envelope study complete: " & Filled & _
...
Err_Handler:
    ...
    Close #FileNum
    Close #OutNum
    MsgBox "F envelope study failed: " & Err.Description, vbExclamation, "F envelope"
End Sub
...
'------------------------------------------------------------------------------
' PURPOSE
'   Returns a usable LOCAL path to f_envelope_grid.csv, or an empty string if
'   the user cancels.
'
' WHY THIS EXISTS
...
'------------------------------------------------------------------------------
    ...
    If Len(BookPath) > 0 And LCase$(Left$(BookPath, 4)) <> "http" Then
        Candidate = BookPath & Application.PathSeparator & "f_envelope_grid.csv"
