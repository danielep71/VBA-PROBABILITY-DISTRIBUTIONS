...
def main():
    ...
    ap.add_argument("--allow-missing-manifest", action="store_true",
                    help="Development mode: warn (don't fail) when the provenance manifest is absent.")
    args = ap.parse_args()

    # PROVENANCE GATE: the committed observations must be bound to the checked-out
    # source that produced them. A green summary otherwise only proves the OLD
    # observations still pass - not that they came from THIS source. A stale or
    # mismatched manifest is a hard failure; an absent one fails unless explicitly
    # allowed for local development.
    from _manifest import repo_root, load_manifest, verify_source_binding, MANIFEST_NAME
    _root = repo_root()
    _manifest = load_manifest(_root)
    if _manifest is None:
        _msg = (f"no {MANIFEST_NAME}: the accuracy summary cannot be bound to the source that "
                "produced the observations. Run write_manifest.py at export time and commit it.")
    ...
    prov = ["Generated by `compute_errors.py` from the regime-aware contract.", ""]
    if _manifest is not None:
        env = _manifest.get("environment", {})
        prov += [
            ...
            "",
            f"- source commit: `{_manifest.get('commit_sha','unrecorded')}`  ",
            f"- observations exported: `{_manifest.get('generated_utc','unrecorded')}`  ",
            f"- Excel: `{env.get('excel_version','unrecorded')}` build "
            f"`{env.get('excel_build','unrecorded')}` ({env.get('office_bitness','unrecorded')}-bit)",
            "",
        ]
    else:
